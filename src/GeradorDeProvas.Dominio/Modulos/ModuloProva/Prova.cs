...
// título, disciplina, matéria*, série 
// e quantidade de questões
public sealed class Prova : EntidadeBase<Prova>, IEntidadeDoUsuario
{
    ...
    public override List<string> Validar()
    {
        ...
        if (ProvaRecuperacao && Materia is not null)
            erros.Add("O campo \"Matéria\" não pode ser preenchido em uma prova de recuperação.");

        else if (!ProvaRecuperacao && Materia is not null && !Equals(Materia.Serie, Serie))
            erros.Add("O campo \"Série\" precisa alinhar com a serie da \"Matéria\".");
    ...
    public List<string> SortearQuestoes(IEnumerable<Questao> questoesDisponiveis, Random? random = null)
    {
        ...
        if (QuantidadeQuestoes < 1)
            erros.Add("A quantidade de questões deve ser maior do que zero.");
